...
/**
 * GameSection - Optimized for Mobile
 * Reduced padding and border radius for small screens to prevent overflow.
 */
function GameSection({ phaseLabel, title, description, color, isLocked, children }: any) {
  ...
  return (
    <div className={`transition-all duration-700 rounded-3xl sm:rounded-[40px] p-4 sm:p-10 border shadow-sm ${
      isLocked 
      ...
      <header className="mb-6 sm:mb-8">
        ...
        <h2 className="font-bold text-3xl sm:text-6xl font-serif tracking-tighter leading-tight">
          {title}
        </h2>
        {!isLocked && <p className="text-sm sm:text-lg text-neutral-500 mt-3 sm:mt-4 leading-relaxed">{description}</p>}
      </header>
      {!isLocked && <div className="mt-2">{children}</div>}
    </div>
  );
}
...
export default function ArcadeManager({ lang, gamesData, dict, demoVideo, offlineTrack }: ArcadeManagerProps) {
  ...
  return (
    <div className="space-y-12 sm:y-16 pb-24 max-w-full overflow-x-hidden">
      {/* SELECTION HUB - Now stacks on Mobile to prevent squashing */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 max-w-4xl mx-auto mb-10 sm:mb-16">
        <button 
          key="select-offline"
      ...
      <div className="space-y-16 sm:space-y-24">
        {gamesData.map((game) => (
          <div key={`container-${game.id}`} className="px-1 sm:px-0">
            <GameSection
              key={`section-${game.id}`}
