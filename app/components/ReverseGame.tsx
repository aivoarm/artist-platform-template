...
export function ReverseGame() {
  ...
  const handleSearch = async () => {
    ...
    // Auto-play reversed when loaded
    playAudio('reversed'); 
  };
  ...
  const handleGuess = (option: any) => {
    ...
    if (option.isCorrect) {
      setGameState('won');
      playAudio('normal'); // Reward: play the real song!
    } else {
      setGameState('lost');
    }
  };

  // ... (Keep loadAndReverseAudio, playAudio, stopAudio functions exactly as they were) ...
        ...
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          {gameState === 'lost' && (
             <div className="mt-6 text-center">
               <h3 className="text-xl font-bold text-red-400 mb-2">❌ Wrong!</h3>
               <button 
                 onClick={handleSearch} 
                 className="text-sm bg-neutral-800 px-4 py-2 rounded-full hover:bg-neutral-700"
               >
                 Try Another Song
               </button>
             </div>
