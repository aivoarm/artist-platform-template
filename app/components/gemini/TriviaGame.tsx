...
// Leveraging your existing dictionary
...
const TriviaGame: React.FC<TriviaProps> = ({ onViewChange }) => {
  ...
  // Shuffles the questions so the game isn't predictable
  const startMatch = () => {
  ...
  const handleAnswer = (idx: number) => {
    ...
    // Check against the 'correct' key in your JSON
  ...
  return (
    <div className="max-w-4xl mx-auto py-10 space-y-8 animate-in fade-in duration-500 relative">
      ...
      >
        ...
        {gameState === 'lobby' ? "Back to Dashboard" : "Quit Session"}
      </button>

      {/* LOBBY VIEW */}
      {gameState === 'lobby' && (
        <div className="glass rounded-[3.5rem] p-16 text-center space-y-8 border border-emerald-500/20 shadow-2xl">
          <h2 className="text-7xl font-black italic tracking-tighter uppercase leading-tight">Offline <br /> Core</h2>
          <p className="text-slate-400 max-w-lg mx-auto leading-relaxed">
            Instant-access knowledge modules synced from the internal dictionary. 
            Zero latency. Total reliability.
          </p>
          <button 
            onClick={startMatch}
            className="px-16 py-6 bg-emerald-500 text-black font-black rounded-3xl hover:scale-105 transition-all shadow-xl"
          >
            START MATCH
          </button>
        </div>
        ...
        <div className="space-y-8">
          ...
          {selectedIdx !== null && (
            <div className="animate-in slide-in-from-top-4 duration-500 p-8 glass rounded-[2.5rem] border border-white/10">
              <p className="text-emerald-500 text-[10px] font-black uppercase tracking-widest mb-2">Production Fact</p>
      ...
      {gameState === 'result' && (
        <div className="glass rounded-[3rem] p-16 text-center space-y-8 border border-white/10 shadow-2xl animate-in zoom-in">
          <p className="text-sm font-black uppercase tracking-[0.3em] text-emerald-500">Sync Complete</p>
